...
func unmarshalQueryJobConfig(structObj *structpb.Struct) (*QueryJobConfig, error) {
	...
	jsonObj, err := marshaller.MarshalToString(structObj)
	if err != nil {
		return nil, err
	}

	return &obj, json.Unmarshal([]byte(jsonObj), &obj)
}
...
func getQueryParameter(literal *flyteIdlCore.Literal) (*bigquery.QueryParameterType, *bigquery.QueryParameterValue, error) {
	...
	return nil, nil, errors.Errorf(pluginErrors.BadTaskSpecification, "unsupported literal [%v]", literal)
}
